...
Proof.
  ...
  - intros m. destruct m.
    + simpl. intros eq. inversion eq.
    + simpl. intros eq. apply f_equal. rewrite <- IHn.
       * reflexivity. 
       * apply eq.
Qed.      
...
Proof.
  ...
  - simpl. destruct n. 
    ...
    + simpl. intro eq. apply IHl. inversion eq. reflexivity.
Qed. 
...
Proof.
    ...
    - simpl. destruct x. destruct (split l). intros l2 l3 h1. 
...
(** **** Exercise: 2 stars (destruct_eqn_practice)  *)  (* self *)
...
Proof.
  ...
  - apply beq_nat_true in bnm. rewrite <- bnm in H0. apply H0.
  - inversion H.
Qed.
...
Proof.
  ...
  rewrite H0 in H.
  rewrite H.
  assert (forall p, beq_nat p p = true). {
    induction p0.
    - simpl. reflexivity.
    - simpl. apply IHp0.
  }
...
Proof.
  ...
  - intros. destruct l2.
    + inversion H. 
    + simpl. inversion H. rewrite (IHl1 l2 H1). reflexivity.
Qed.
(* FILL IN HERE *) 
(* Do not modify the following line: *)
...
Proof.
  ...
  - simpl. intros. destruct (test x) eqn:Heqe.
